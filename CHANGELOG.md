...
- New `ScenarioCategory::Writing` category and `scenarios/en/writing/` scenario track (#152, #361) covering the Markdown prose-editing use cases from issue #152's request: bold/italic emphasis via `ms` (`emphasis.toml`), Markdown link construction via `miw`/`ms[`/append (`links.toml`), list item indent/outdent via `>`/`<` (`list-indent.toml`), heading level changes via insert/delete (`heading-level.toml`), blockquote prefixing via `I` (`blockquote.toml`), and code-fence wrapping via `O`/`o` (`code-fence.toml`). All six scenarios set `setup.language = "md"` to render with Markdown syntax highlighting; Typst content and "move paragraph"/"list-to-numbered-list" use cases are explicitly out of scope for this pilot
- Python-variant pilot scenarios validating non-Rust scenario content (#362): `scenarios/en/movement/word-python.toml` and `scenarios/en/editing/indentation-python.toml`, both setting `setup.language = "py"`. No `ScenarioCategory` variant was added — the pilot reuses the existing `movement`/`editing` categories (SC-004)
...
- `ChallengeProgress`'s day-rollover check (comparing `last_challenge_date` against the current date) was independently re-derived in `can_attempt`, `attempts_remaining`, `start_attempt`, and `is_today`; extracted into a single private `is_new_day` helper all four now call. No behavior change. The struct's pre-existing `TODO: CR-013` (deferred migration from a stringly-typed date to `chrono::NaiveDate`) now references this issue as its tracking issue (#391)
- **BREAKING**: MSRV bumped to 1.91, required to use `Path::with_added_extension`, which replaces manual `OsString` temp-file-name construction in `ProfileStorage::save`/`ProfileStorage::refresh_lock`/`ProfileStorage::lock_path` (`src/gamification/storage.rs`) with a single shared `tmp_write_path` helper; no behavior change, temp/lock file paths are identical to before for every path this crate constructs (#378)
- `deny.toml`'s `[bans].skip` now documents and skips the 24 remaining duplicate-version clusters surfaced by `cargo deny check bans` beyond the `windows_x86_64_*`/`winnow` pair already tracked (#356/#369): `bitflags`, `darling`/`darling_core`/`darling_macro`, `foldhash`/`hashbrown`, `getrandom`/`r-efi`, `itertools`, `serde_spanned`/`toml`/`toml_datetime`/`toml_edit` (same toml-0.8-vs-1.x split as `winnow`), `syn`, `thiserror`/`thiserror-impl`, `unicode-width`, and `windows-sys`/`windows-targets`/the `windows_{aarch64,i686}_*` counterparts of the `windows_x86_64_*` crates (same four transitive chains, now covering all target architectures). Each is an independent transitive dependency chain with no compatible newer version to converge on (verified per-crate via `cargo update -p <crate> --dry-run`); the one exception, `darling` bumping to 0.24.0 through ratatui's `instability` crate, does not collapse the duplicate (`derive_builder`'s chain still pins `darling` `^0.20`) and was not taken. No CVEs are involved; `multiple-versions` stays "warn" (#370)
- **BREAKING**: Renamed 9 public getters to drop the redundant `get_` prefix per Rust API Guidelines [C-GETTER](https://rust-lang.github.io/api-guidelines/naming.html#getter-names-follow-rust-convention-c-getter) (#359): `HelixSimulator::get_state`/`AnyModeSimulator::get_state` -> `state`, `CommandRegistry::get_metadata` -> `metadata`, `GameSession::get_hint` -> `hint`, `PerformanceTracker::get_stats_clone` -> `stats_clone`, `PerformanceTracker::get_performance` -> `performance`, `PerformanceTracker::get_weak_commands` -> `weak_commands`, `Scorer::get_rating` -> `rating`, `AppState::get_scenario` -> `scenario`. Parameterized lookups (`get_by_id`, `get_by_difficulty`, `get_due_reviews`, `get_embedded_scenarios`) are unaffected. This rename surfaced a pre-existing duplication between the new `state()` and `to_editor_state()` (a same-purpose alias predating this change); `CommandExecutor::to_editor_state` (the trait `AnyModeSimulator` implements) is now `CommandExecutor::state`, and the redundant `to_editor_state`/`state` alias pairs on `HelixSimulator`/`AnyModeSimulator` are collapsed to a single `state()` each
- **BREAKING**: Renamed 3 more `ScenarioCollection` getters to drop the redundant `get_` prefix, same C-GETTER rationale as #359 (#375): `get_filtered` -> `filtered`, `get_categories` -> `categories`, `get_difficulties` -> `difficulties`. The parameterized `get_filtered_by_index` lookup is unaffected
...
- Follow-up to #399's `change_selection`/`delete_selection` fixes (#395/#396): (1) linewise `c` (e.g. `xc`, selecting a whole line via `x` then changing it) now replaces the selection with a blank line and enters Insert mode there, matching upstream Helix's `change_selection` falling back to `open(Open::Above)` for a whole-line selection, instead of leaving a plain mid-line insertion point at the deletion offset. New `is_selection_linewise`/`change_selection_linewise` (`src/helix/simulator/commands/editing.rs`), shared by `change_selection` and `change_selection_noyank`. (2) The blackhole register (`"_`) is now supported: `RegisterFile::set` silently discards any write addressed to it (`BLACKHOLE_REGISTER`), and the `"<reg><op>` register-scoped input path (`RegisterOpPending`, `KeyTrie`) now also accepts `d`/`c` as operators (previously only `y`/`p`/`P`/`R`), so `"_d`/`"_c` delete/change without writing to any register, and `"<reg>d`/`"<reg>c` route to a named register like the existing `y`/`p`/`P`/`R` ops. `cmd_to_key_events`/`execute_key_sequence` (`src/helix/simulator/commands/mod.rs`, `src/helix/simulator/mod.rs`) gained a matching 3-key arm for the `"<reg><op>` shape, since without it `.` (repeat) silently recorded nothing for these register-scoped commands and replayed a stale prior action instead. (3) A new `Alt-d` binding (`delete_selection_noyank`) was added, mirroring upstream Helix's `default.rs` keymap, which binds `A-d` alongside the already-implemented `A-c` (`change_selection_noyank`) - both delete/change without writing to any register, independent of and in addition to the new blackhole-register path from item 2. (4) `enter_insert_mode` still collapses every selection to a single primary cursor before Insert mode starts (documented as a known, pre-existing limitation): this simulator's Insert-mode commands (`insert_text`, `backspace`, arrow-key handlers) all read/write only `selection.primary()` throughout typing, not just at entry, so preserving the full selection there alone wouldn't make multi-cursor typing actually work - a real fix needs those commands reworked to operate over every range, out of scope here (#400)
- Arcade/Survival/Challenge stats bar (`render_stats_bar`, `src/ui/render/minigame.rs`) silently clipped its second HUD line (COMBO/STREAK/difficulty level+tier/BEST) at every terminal size: the bordered `Paragraph` has 2 content lines, needing height 4 (1 top border + 2 content + 1 bottom border), but the countdown, playing, and paused screens all allocated only `Constraint::Length(3)` for that layout chunk. All 3 call sites now allocate `Constraint::Length(4)` (#393)
- `scripts/install.sh`'s exarch bootstrap (`resolve_exarch`) downloaded and executed a third-party shell script (`exarch`'s own installer) from the `main` branch with no checksum, signature, or version pin, unlike the helix-trainer archive download it sits alongside, which already verifies a SHA-256 checksum before extracting. The bootstrap script is no longer used at all: `resolve_exarch` now downloads a specific, hardcoded `exarch` release version (`EXARCH_VERSION`) and verifies it against a SHA-256 checksum pinned as a constant in `install.sh` itself, per target (`EXARCH_SHA256_LINUX_X86_64`/`_LINUX_AARCH64`/`_MACOS_X86_64`/`_MACOS_AARCH64`) — not resolved from exarch's own "latest" release or its release-published checksum at install time, since fetching the checksum from the same release as the binary it verifies would only guard against corruption, not a compromised release. Trust for the exarch binary is now anchored entirely in this repo, itself fetched at a pinned release tag. Checksum verification (`verify_checksum`, now factored out and shared with the helix-trainer archive's own verification) was also hardened to always do an explicit digest string-compare requiring the checksum file to be non-empty and reference the exact archive filename, rather than trusting a checksum tool's exit code — `sha256sum -c` on some platforms (e.g. macOS's `/sbin/sha256sum`) exits 0 given a checksum file with no parseable lines, which would otherwise let a truncated or empty download through as "verified"; `scripts/install.ps1`'s own checksum check gained the same filename-binding for consistency, though it already failed closed on an empty checksum file. Separately, the top-level installer one-liners in README.md had the same moving-ref trust problem — piping `raw.githubusercontent.com/.../main/install.sh` — and now resolve and pin to the latest release tag first, and download the script to a freshly-generated temp file (`mktemp` / `[System.IO.Path]::GetRandomFileName()`) before running it, instead of piping directly or writing to a fixed, predictable path: piping silently no-ops on a failed download since a nonzero `curl` exit doesn't stop `sh` from processing empty stdin as a successful no-op script, and a fixed path in a world-writable directory like `/tmp` can be pre-planted as a symlink to overwrite an unrelated file. The bash one-liner runs the download-and-execute chain inside a subshell with a `trap ... EXIT` cleanup instead of a trailing `; rm -f`, so a failed `curl`/`sh` still propagates its real exit status to the caller (e.g. under `set -e` or CI exit-code gating) instead of always reporting success via the cleanup command's own exit code. `install.sh`/`install.ps1` also moved from the repo root to `scripts/` (#392)
- `map_macos_composed_char` (`src/input/typestate/key_mapping.rs`) mismapped two macOS Option composition entries to the wrong base key: `˝` (U+02DD, double acute accent) was mapped to Alt-J instead of Alt-G, and `¯` (U+00AF, macron) was mapped to Alt-_ instead of Alt+Shift+, (`<`, to avoid aliasing the existing Alt-, entry for `≤`). Verified against the actual macOS "U.S." system keyboard layout (`/System/Library/Keyboard Layouts/U.S.keylayout`): on the standard US layout, Option+Shift+G composes `˝` and Option+Shift+, composes `¯`; Option+Shift+J instead composes `Ô` and Option+Shift+- composes `—` (em dash), neither of which is mapped through this table (#382)
- `ç` and `ß` were unconditionally rewritten to Alt-c/Alt-s by `map_macos_composed_char`, but both are plain, directly-typeable keys on real keyboard layouts other than US - `ç` on French AZERTY and Portuguese/Brazilian ABNT, `ß` on German QWERTZ - so a real keystroke on those layouts was indistinguishable from macOS's US Option-c/Option-s composition artifact (same class of bug as the backtick fix below, #377). Both match arms are removed; `ç`/`ß` now pass through unmodified regardless of keyboard layout. Trade-off (accepted, tracked separately): this makes Alt-c/Alt-s unreachable via Option-key composition on macOS terminals without kitty keyboard protocol support (e.g. Terminal.app), which specifically breaks `scenarios/en/selection/advanced-selection.toml`'s Alt-s step for those users (#386). `'Ç' => ('C', ALT)`, `'¬' => ('l', ALT)`, and `'˚' => ('k', ALT)` have the same class of collision on other layouts (e.g. Ç is a plain Shift+key on Brazilian ABNT2/Portuguese, not just French AZERTY as previously assumed; ¬ is Shift-` on British PC) and are intentionally left unaddressed here, out of scope for this fix (#381)
- `Alt-s` (split-selection-on-newline) being unreachable via Option+s on macOS Terminal.app and other non-kitty terminals (the accepted #381/#382 trade-off above) was undiscoverable: no documented workaround existed, even though one already shipped in #350. README's "Custom Helix keymap" section now documents remapping `Alt-s` to a plain key (e.g. `Ctrl-y`) via `[keys.normal]`, and `scenarios/en/selection/advanced-selection.toml`'s `split_selection_newlines_001` hints point affected players at it. No behavior change; the trade-off itself is unchanged. `Alt-c` (`CMD_CHANGE_SELECTION_NOYANK`) needs no remap workaround here because it currently has no executable command behind it at all - not a keymap-reachability issue, tracked separately (#389) (#386)
- `Alt-c` (change selection without yanking) reached `CommandRegistry::execute()` with no matching entry and returned an unknown-command error on every genuine keypress, even though `key_mapping.rs` already mapped it to `CMD_CHANGE_SELECTION_NOYANK` correctly. `src/helix/registry/definitions/editing.rs` now registers `change_selection_noyank` for that command, reusing the existing `HelixSimulator::change_selection` handler verbatim - this simulator never writes to the yank register from change/delete operations, so yank and no-yank variants are already behaviorally identical here. `is_insert_command` also now recognizes the new command, which is what drives the Normal-to-Insert mode transition on execution - without it Alt-c would delete but leave the simulator in Normal mode (#389)
- `c` (change selection) deleted only the single character under the cursor instead of the full active selection range, and did not properly collapse the selection before entering insert mode (#395). Change/delete commands (`c`, `d`) never wrote the deleted text to any register, so a subsequent `p`/`P` pasted nothing back (#396). Fixed by extracting a shared `delete_active_selection` helper (`src/helix/simulator/commands/editing.rs`) used by both `delete_selection` (`d`) and the new `change_selection_impl` (`c`/`Alt-c`): it deletes the full active selection - respecting multi-range selections - via the existing `Transaction::change_by_selection` pattern, then maps the resulting selection through the transaction's changes (`helix_core::Selection::map`) instead of a stale pre-transaction cursor offset, so a multi-range deletion (e.g. from `C`) lands the cursor at its correct post-deletion position rather than past the end of the document. `delete_selection` and `change_selection` now write the deleted text to the default register via the existing `yank_to_register` helper (`clipboard.rs`), which itself now folds over every selection range instead of only the primary one, so a multi-cursor `d`/`c` captures all deleted fragments, not just one. `change_selection_noyank` (`Alt-c`) is now registered against its own handler (`src/helix/registry/definitions/editing.rs`) that performs the same deletion without the register write, since the shared-handler rationale from #389 no longer held once `change_selection` started writing registers. `d` stays in Normal mode and clamps each resulting cursor to the last valid character; `c`/`Alt-c` hand the raw mapped position to Insert mode, matching `append`'s existing boundary handling
- A plain, unmodified `` ` `` keypress was being silently rewritten to Alt-`` ` `` before it ever reached command dispatch, so it always resolved to `switch_to_uppercase` instead of `switch_case_alt`. `map_macos_composed_char` (`src/input/typestate/key_mapping.rs`), used by both the live per-keystroke path and the custom-keymap overlay path, unconditionally treated every `` ` `` char as a macOS Option-`` ` `` dead-key composition artifact and injected the ALT modifier regardless of the `KeyEvent`'s real modifiers; unlike its other table entries, backtick is also a directly-typeable ASCII key, so the rewrite collided with real keystrokes. The `` ` `` -> Alt-`` ` `` match arm is removed; a bare `` ` `` now resolves to `switch_case_alt` and a real Alt-`` ` `` to `switch_to_uppercase`. Trade-off (unverified): if a terminal ever delivers a bare backtick for a released Option-`` ` `` dead key, it will now read as plain `` ` `` (`switch_case_alt`) instead of the previous `switch_to_uppercase` guess (#377)
- `ProfileStorage::load` and `ConfigStorage::load` now reject an oversized `profile.json`/`config.json` (new `MAX_PROFILE_FILE_SIZE`/`MAX_CONFIG_FILE_SIZE`, 1 MB each) before reading the file into memory, via the same `path_validator::validate_file_size` guard already used for scenario TOML and Helix keymap files, instead of unconditionally reading the whole file with no upper bound (#357)
- The Target panel's syntax highlighting no longer hardcodes Rust regardless of scenario content: `Setup` gained an optional `language` field (file-extension-style token, e.g. `"md"`, `"py"`) resolved by `highlight_code_with_multi_cursor`, falling back to plain text (never to Rust highlighting) when unset or unresolvable. All 149 existing scenarios omit the field and continue to render identically, defaulting to `"rs"` (#360)
- Two pairs of scenario TOML entries shared the same `id` despite being genuinely different scenarios: `delete_line_001` (`scenarios/en/basic/delete.toml`'s "Delete current line" vs. `scenarios/en/editing/delete-selection.toml`'s "Delete entire line") and `join_lines_001` (`scenarios/en/editing/join.toml`'s "Join two lines" vs. `scenarios/en/editing/advanced-editing.toml`'s "Join two lines"). Since `ScenarioHistory::completions` is keyed by this id string, both scenarios in a pair loaded and appeared as separate, independently playable menu entries, but playing either one silently marked its sibling as completed/mastered even though it was never played - corrupting `ScenarioCollection::is_curriculum_complete()`, the End-Game Summary's mastery-by-category breakdown, and per-scenario attempts/best_score/mastery_level bookkeeping. Renamed the single-scenario side of each pair to a distinct id (`delete_line_001` -> `delete_current_line_001` in `basic/delete.toml`; `join_lines_001` -> `join_lines_basic_001` in `editing/join.toml`), keeping the other side's id and the numbered sequences in `delete-selection.toml`/`advanced-editing.toml` unchanged. `cargo nextest run scenario` now includes a `test_all_scenario_ids_are_unique` check that fails with the offending ids if a future scenario reintroduces a duplicate, and a `test_quest_scenario_references_exist` check that cross-references every quest's `scenario_id`/`requires_scenarios` entries against the loaded scenario collection, catching a dangling quest reference if a scenario id is ever renamed or removed again. `quests/en/daily.toml`'s `speed_delete_hard` quest references `scenario_id = "delete_line_001"`, which after this rename resolves unambiguously to the surviving `delete-selection.toml` scenario ("Delete entire line") it always targeted (previously ambiguous between the two same-id scenarios). Note: the old ids are not orphaned - they remain live keys, now solely attributed to their surviving sibling scenario. A player who has only completed the *renamed-away* scenario (e.g. `basic/delete.toml`'s "Delete current line") will, after upgrading, see the surviving sibling ("Delete entire line") falsely showing as completed/mastered under the reused id, while their actual completion of the renamed scenario is lost (nothing recorded under its new id). No migration is provided for this local `profile.json` re-attribution (#363)
- The key-history popup (`render_key_history_popup`) could corrupt the Target State panel's border in Training mode whenever a wide key history was showing - reproducible via any multi-character pending-input prompt (`s`/`S` regex-selection, `q`/`Q` macro record/replay, named-register `"a`-prefixed operations). The popup was positioned as an independent overlay sized/placed relative to the whole frame (`reserved_bottom: u16`, a fixed row count reserved above the bottom HUD bars), with no awareness of the Target panel's own border falling inside that same region, so a wide key history's popup box could draw its own border directly across the panel's border row, doubling/misaligning box-drawing characters. `render_editor_pair` now returns the Target panel's outer `Rect`, and `render_key_history_popup` takes that `Rect` as its `bounds` parameter instead of a bare row count: it is anchored to the bottom-right corner of `bounds`'s *inner* area (inset from the border) and its width is clipped to fit entirely within that inner area, so it can never draw over the Target panel's border regardless of how wide the key text grows - in both Training and Arcade mode. If the panel's inner area is smaller than the popup's minimum legible size (`KEY_HISTORY_MIN_WIDTH` width, half the `tui-big-text` glyph height), rendering is skipped entirely for that frame instead of drawing an illegibly squished/clipped glyph. Training mode's Task screen also now renders the key-history popup before the hint popup, so the hint popup (which can be wide enough to reach the same corner) draws on top of it instead of the reverse. Previously tracked (for Arcade mode only) as #272 (#364)
...
- Daily Challenge's advertised 3-attempts-per-day limit is now actually enforced: `ChallengeProgress` (`can_attempt`/`start_attempt`/`record_result`) was fully implemented and unit-tested but had no production caller, so launching the mode never checked or consumed an attempt and completing one never recorded a result. `handle_launch_minigame_mode` now checks `can_attempt` before creating a session (showing an info notification and staying on the mode-selection screen once attempts are exhausted for the day); the attempt is only consumed (`start_attempt` + an immediate, non-debounced save) once the launch is actually going to succeed, i.e. after the empty-scenario-pool guard, so a launch that can't start never burns one of today's 3 attempts. `handle_minigame_game_over` now calls `record_result` for Challenge-mode sessions, covering every exit path (abandon, timeout, session-timeout, back-to-menu) since they all funnel through it. The mode-selection submenu now shows attempts remaining today next to the Daily Challenge entry as "(N/3)" on the mode name line (`ChallengeProgress::attempts_remaining`) - appending it to the description line instead, as first implemented, silently overflowed the submenu's fixed 43-column inner width and never reached the screen. Reads `state.progress.profile.challenge_progress` directly at render time; the now-redundant `MiniGameData.challenge_progress` snapshot field (which would otherwise go stale after `record_result`, and had no other reader) was removed (#376)
- The mode-selection submenu's 3rd entry (Daily Challenge) never rendered at the documented minimum terminal size of 80x24: `render_minigame_mode_submenu`'s height clamp (`parent_area.height.saturating_sub(3)`) left only 5 inner rows, but each entry's 2-line (name + description) layout at 3-row spacing needed 8 rows to fit all 3. Entries are now single-line (the per-entry description was dropped) at 2-row spacing, fitting all 3 within 5 rows with no slack; `submenu_height` is now derived from `MiniGameModeSelection::MODE_COUNT` instead of a hardcoded `11`, so it scales if a mode is ever added or removed. Also fixed a latent rendering bug surfaced while verifying this at 80x24: the submenu's background was "cleared" via `Block` styling alone, which restyles cells but never overwrites their symbols, so a stale row from the parent screen could bleed through the submenu; a `ratatui::widgets::Clear` is now rendered first (#385)
