...
fn test_typestate_handler_state_name() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('g'), KeyModifiers::NONE));
...
fn test_typestate_handler_match_transitions() {
    ...
    // m -> MatchPending
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('m'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_match_to_surround_add() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('s'), KeyModifiers::NONE));
    state = next;
    assert_eq!(state.state_name(), "SURROUND_ADD_PENDING");

    // ( -> execute surround add
    let (result, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('('), KeyModifiers::NONE));
    assert!(result.is_execute());
    state = next;
    assert!(state.is_base());
}
...
fn test_typestate_handler_match_to_surround_delete() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('d'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_match_to_surround_replace() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('r'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_match_to_text_object_around() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('a'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_match_to_text_object_inside() {
    ...
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('i'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_find_char() {
    ...
    // f -> FindCharPending
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('f'), KeyModifiers::NONE));
    state = next;
...
fn test_typestate_handler_count_pending() {
    ...
    // 5 -> CountPending
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('5'), KeyModifiers::NONE));
    state = next;
    assert_eq!(state.state_name(), "COUNT_PENDING");

    // 3 -> still CountPending (53)
    let (_, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('3'), KeyModifiers::NONE));
    state = next;
    assert_eq!(state.state_name(), "COUNT_PENDING");

    // j -> execute with count
    let (result, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE));
    assert!(result.is_execute());
...
fn test_typestate_handler_unmatched_prev() {
    ...
    let (result, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('p'), KeyModifiers::NONE));
    assert!(result.is_execute());
    state = next;
    assert!(state.is_base());
}
...
fn test_typestate_handler_unmatched_cancel() {
    ...
    let (result, next) =
        state.process_key(KeyEvent::new(KeyCode::Char('x'), KeyModifiers::NONE));
