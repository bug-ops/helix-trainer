...
- New scenarios covering previously-untested Normal Mode commands (#198): `scenarios/en/movement/scroll.toml` (`Ctrl-d`/`Ctrl-u`/`Ctrl-f`/`Ctrl-b` half-page/page movement, including document-boundary clamping) and `scenarios/en/selection/select-all-replace.toml` (`%` select-all, `R` replace-selection-with-yanked). `zz`/`zt`/`zb` (view alignment) and `q`/`Q` (macros) are not scoreable/implemented in this simulator and are intentionally omitted; `s`/`S` (regex select/split) are documented as commented-out placeholders pending their still-stubbed simulator implementation. `x`/`X` (line selection) and `ge` (Helix's "go to last line", the equivalent of vim's `G`, which this keymap does not bind) already had scenario coverage
