...
fn test_complete_learning_workflow() {
    // 1. Create shared performance tracker
    ...
    // Verify initial state
    {
        ...
        let yy_perf = tracker_ref.get_performance("yy").unwrap();
        assert_eq!(yy_perf.attempts, 3);
        assert_eq!(yy_perf.successes, 2);

        let w_perf = tracker_ref.get_performance("w").unwrap();
    ...
    let due_reviews = scheduler.get_due_reviews();
    ...
    // Complete all reviews (if there are any)
    let mut completed = 0;
    while let Some(_item) = session.current_item() {
        session.record_and_next(Duration::from_millis(300), true, Duration::from_millis(500));
        completed += 1;
    }

    // Session should be complete after processing all items
    assert!(session.is_complete());

    // If we had items, verify summary
    if completed > 0 {
        let summary = session.summary();
        assert_eq!(summary.total_reviews, completed);
    }

    // 5. Check analytics
    let analytics = Analytics::new(Rc::clone(&tracker));

    let mastery_summary = analytics.get_mastery_summary();
    ...
    let progress = analytics.get_progress_over_time(7);
...
fn test_scheduler_prioritization() {
    ...
    let session = scheduler.recommend_practice_session(15);
